...
const Scene = () => {
    ...
    const [_character, setChar] = useState(null);
    useEffect(() => {
        if (canvasDiv.current) {
            ...
            loadCharacter().then((gltf) => {
                if (gltf) {
                    ...
                    // Store handler reference so we can properly remove it on cleanup (was leaking before)
                    const onResize = () => handleResize(renderer, camera, canvasDiv, character);
                    window.addEventListener("resize", onResize);
                    // Store on local var for cleanup below
                    window.__portfolioResizeHandler__ = onResize;
                }
            });
            ...
            return () => {
                ...
                scene.clear();
                renderer.dispose();
                // Remove stored resize handler reference (prevents memory leak)
                const storedHandler = window.__portfolioResizeHandler__;
                if (storedHandler) {
                    window.removeEventListener("resize", storedHandler);
                    delete window.__portfolioResizeHandler__;
                }
                if (canvasDiv.current) {
